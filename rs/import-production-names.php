...
$csvfile = $_SERVER["DOCUMENT_ROOT"]."/upload/parse/production-names.csv";
...
$i = 0;//для отладки
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "START_IMPORT_PRODUCTION_NAMES","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Обновление название товаров производителей. СТАРТ"));
...
while (($line = fgetcsv($file)) !== FALSE) {
	...
	if($line[0]) {
		...
		$senderCode = $arProduct[0];
		if($arProductsInCatalog[$senderCode]) {//если товар есть в ИБ


			CIBlockElement::SetPropertyValuesEx($arProductsInCatalog[$senderCode]['ID'], false, array('PRODUCTION_NAME' => $arProduct[2]));
			$i++;
...
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "END_IMPORT_PRODUCTION_NAMES","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Обновлены названия товаров производителей. Всего: ".$i));
