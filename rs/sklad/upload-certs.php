...
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "RS_CERTS_IMPORT_START","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => ""));
...
$arcSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_SenderPrdCode", "PROPERTY_DATE_UPDATE_CERTS");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
$arcFilter = Array("IBLOCK_ID"=>$IBLOCK_ID, "PROPERTY_Certificate" => false);
...
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "UPLOAD_CERTS_RS_START","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Загрузка сертификатов. СТАРТ . Элементов для обработки: ".count($arProductsInCatalog)));
...
while ($reader->read()) {
    if($reader->nodeType == XMLReader::ELEMENT && $reader->name == 'DocDetail' ) {
		...
		$hasCerts = (count($product->CertificateInfo->Certificate) > 0) ? 'Y' : 'N';//
		if(isset($arProductsInCatalog[$senderCode]) && $hasCerts == 'Y') {//если товар есть в инфоблоке и есть сертификаты

			$productUpdateDate = IntegrationRS::getProductUpdateCertsDate($senderCode, $arProductsInCatalog);
			if($productUpdateDate != $date) {//прогружаем свойства только если они еще не добавлялись
				//echo '<pre>'.print_r($product,true).'</pre>';
				IntegrationRS::updateProductCerts($productId, $product, $IBLOCK_ID, $ADMIN_ID, $date);
				$i++;
			}
...
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "UPLOAD_CERTS_RS","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Загружены сертификаты. Всего товаров: ".$i));
