...
$csvfile = $_SERVER["DOCUMENT_ROOT"]."/upload/parse/video.csv";
...
$i = 0;//для отладки
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "START_IMPORT_YOUTUBE_RS_PRODUCTS","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Обновление YOUTUBE. СТАРТ"));
...
while (($line = fgetcsv($file)) !== FALSE) {
	...
	if($line[0] && $line[5]) {
		...
		if($arProductsInCatalog[$senderCode]) {//если товар есть в ИБ
			...
			CIBlockElement::SetPropertyValuesEx($arProductsInCatalog[$senderCode]['ID'], false, array('VIDEO_YOUTUBE' => $line[5]));
			$i++;
...
CEventLog::Add(array("SEVERITY" => "DEBUG","AUDIT_TYPE_ID" => "END_IMPORT_YOUTUBE_RS_PRODUCTS","MODULE_ID" => "iblock","ITEM_ID" => "","DESCRIPTION" => "Обновлены YOUTUBE товаров. Всего: ".$i));
