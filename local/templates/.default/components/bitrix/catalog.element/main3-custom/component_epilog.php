...
<?if($arResult["ID"]):?>
	...
	$bShowDetailTextTab = ($bShowDetailText || $bShowAdditionalGalleryTab ? ++$i : '');
						...
						<div class="ordered-block js-store-scroll tabs-block">
											...
											<?if($value == "desc"):?>
												<?if($bShowDetailTextTab || ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab)):?>
													<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
														<a href="#desc" data-toggle="tab"><?=($arParams["T_DESC"] ? $arParams["T_DESC"] : Loc::getMessage("T_DESC"));?></a>
													</li>
												<?endif;?>
												<?if($bShowDocs):?>
													<li class="<?=(!($iTab++) ? ' active' : '')?>">
														<a href="#files" data-toggle="tab">Сертификаты</a>
													</li>
												<?endif;?>
											<?endif;?>
											<?//show char block?>
							...
							<div class="tab-content<?=($i <= 1 ? ' not_tabs' : '')?>">
								...
								<?foreach($arTabOrder as $value):?>
									...
									<?//files?>
									<?if($bShowDocs):?>
										<div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="files">
											<div class="ordered-block">
												<div class="ordered-block__title option-font-bold font_lg">
...
<?if(is_array($arResult['RES_MOD']['Analog']))://АНАЛОГИ?>
<?
$GLOBALS['rsAnalogFilter'] = array("ID" => $arResult['RES_MOD']['Analog']);
?>
				<section class="specials block_v detailed-block maxwidth-theme bottom-info-wrapper">
					<div class="wraps">
						<h2><?=GetMessage("ANALOG")?></h2>
...
<?endif;?>
<?if(is_array($arResult['RES_MOD']['RelatedProd']))://Сопутствующие?>
<?
$GLOBALS['rsRelatedFilter'] = array("ID" => $arResult['RES_MOD']['RelatedProd']);
?>
				<section class="specials block_v detailed-block maxwidth-theme bottom-info-wrapper">
					<div class="wraps">
						<h2><?=GetMessage("ALSO_BUY")?></h2>
						<div class="ajax_load table">
								<?$isAjax="N";?>
								<?if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == "xmlhttprequest"  && isset($_GET["ajax_get"]) && $_GET["ajax_get"] == "Y" || (isset($_GET["ajax_basket"]) && $_GET["ajax_basket"]=="Y")){
									$isAjax="Y";
...
<?if($USER->IsAdmin()):?>
<div id="purshace_price_out">
	<div class="price_group">
		<div class="price_name font_xs darken"><?=GetMessage("ZAKUP_PRICE")?></div>
		<div class="price_matrix_wrapper ">
			<div class="prices-wrapper">
				<div class="price font-bold font_mxs dd">
					<span>
						<span class="values_wrapper">
							<span class="price_value"><?=$arResult['RES_MOD']['CATALOG_PURCHASING_PRICE'];?></span>
