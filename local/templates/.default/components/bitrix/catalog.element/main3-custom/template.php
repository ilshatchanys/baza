...
$measureId = ($arResult["CATALOG_MEASURE"]) ? $arResult["CATALOG_MEASURE"] : false;

$templateData['QUANTITY_DATA'] = $arQuantityData = CMaxCustom::GetQuantityArray($totalCount, $measureId, $arItemIDs["ALL_ITEM_IDS"], ($arParams["USE_STORE"] == "Y" && $arResult["STORES_COUNT"] && $arResult['CATALOG_TYPE'] != CCatalogProduct::TYPE_SET && (!$arResult["OFFERS"] || ($arResult["OFFERS"] && $arParams["TYPE_SKU"]!="N")) ? "Y" : "N"));
						...
						<div class="product-chars flex-50">
							...
							<?if( ($arResult['DISPLAY_PROPERTIES'] || $arResult['DISPLAY_PROPERTIES_OFFERS']) && $arParams['VISIBLE_PROP_COUNT'] > 0 ):?>
								<div class="char-side">
									...
									<div class="package">
										<?if($arResult['PROPERTIES']['Multiplicity']['VALUE'] || $arResult['PROPERTIES']['UOM']['VALUE']):?>
											<?if(CModule::IncludeModule("catalog")){
												 $m_list = CCatalogMeasure::getList(array(),array('SYMBOL_LETTER_INTL'=>$arResult['PROPERTIES']['UOM']['VALUE']));

												 while($ar_result = $m_list->GetNext()){
													 $measureName = $ar_result['MEASURE_TITLE']; 
													 $measureNameShort = $ar_result['SYMBOL_RUS']; 
												 }
											}?>
											<?if($arResult['PROPERTIES']['ED_IZM_PACK']['VALUE'] && 1 === preg_match('~[0-9]~', $arResult['PROPERTIES']['ED_IZM_PACK']['VALUE'])):?>
												<div class="package-per-unit">
													Кол-во в упаковке: <b><?=$arResult['PROPERTIES']['ED_IZM_PACK']['VALUE']?></b>
												</div>
											<?endif;?>
											<?if(!$arResult['PROPERTIES']['ED_IZM_PACK']['VALUE'] && $arResult['PROPERTIES']['Multiplicity']['VALUE'] && intval($arResult['PROPERTIES']['Multiplicity']['VALUE']) > 1):?>
												<div class="package-per-unit">
													Кол-во в упаковке: <b><?=intval($arResult['PROPERTIES']['Multiplicity']['VALUE'])?> <?=$measureNameShort?></b>
												</div>
											<?endif;?>
											<?if($arResult['PROPERTIES']['UOM']['VALUE']):?>
												<div class="package-unit">
													Единица измерения: <b><?=$measureName?></b>
												</div>
											<?endif;?>
										<?endif;?>
									</div>
												...
												<?else:?>
													<?if(isset($arResult['PRICE_MATRIX']) && $arResult['PRICE_MATRIX']): // USE_PRICE_COUNT?>
														...
														<?//if(count($arResult['PRICE_MATRIX']['ROWS']) > 1 || count($arResult['PRICE_MATRIX']['COLS']) > 1):?>
															<?=CMaxCustom::showPriceMatrix($arResult, $arParams, $strMeasure, $arAddToBasketData);?>
														<?//endif;?>
					...
					<?foreach($arResult['VIDEO'] as $v => $value):?>
						<div class="col-sm-6">
							...
							<div class="youtube-video">
								<?$value = str_replace(array('https://www.youtube.com/watch?v=','https://youtu.be/'),array('',''),$value);?>
								<iframe src="https://www.youtube.com/embed/<?=$value;?>" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>
							</div>
						</div>
					<?endforeach;?>
				<?else:?>
					<div class="col-md-12">
						<?//=$arResult['VIDEO'][0]?>
						<div class="youtube-video">
							<?$arResult['VIDEO'][0] = str_replace(array('https://www.youtube.com/watch?v=','https://youtu.be/'),array('',''),$arResult['VIDEO'][0]);?>
							<iframe src="https://www.youtube.com/embed/<?=$arResult['VIDEO'][0];?>" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>
						</div>
					</div>
				<?endif;?>
			</div>
