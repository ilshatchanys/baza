...
foreach($arResult["DISPLAY_PROPERTIES"] as $propCode => $arProp)
{
	if($arProp['VALUE'] == '' || $arProp['VALUE'] == ' ') {
		unset($arResult["DISPLAY_PROPERTIES"][$propCode]);
	}
}

//Аналоги
if(is_array($arResult['PROPERTIES']['Analog']['VALUE'])) {
	$arSelect = Array("IBLOCK_ID", "ID");
	$arFilter = [
		"IBLOCK_ID"                 => $arParams['IBLOCK_ID'], 
		"ACTIVE"                    => "Y",
		"PROPERTY_SenderPrdCode"    => $arResult['PROPERTIES']['Analog']['VALUE'],
	];
	$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
	while($ob = $res->GetNextElement()){ 
		$arFields = $ob->GetFields();
		$arResult['RES_MOD']['Analog'][] = $arFields['ID'];
	}
}
//Сопутствующие
if(is_array($arResult['PROPERTIES']['RelatedProd']['VALUE'])) {
	$arSelect = Array("IBLOCK_ID", "ID");
	$arFilter = [
		"IBLOCK_ID"                 => $arParams['IBLOCK_ID'], 
		"ACTIVE"                    => "Y",
		"PROPERTY_SenderPrdCode"    => $arResult['PROPERTIES']['RelatedProd']['VALUE'],
	];
	$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
	while($ob = $res->GetNextElement()){ 
		$arFields = $ob->GetFields();
		$arResult['RES_MOD']['RelatedProd'][] = $arFields['ID'];
	}
}

$arResult['RES_MOD']['CATALOG_PURCHASING_PRICE'] = $arResult['CATALOG_PURCHASING_PRICE'];

//Кешируем и перехватываем в component_epilog
