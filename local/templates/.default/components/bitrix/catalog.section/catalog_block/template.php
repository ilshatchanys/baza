...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$measureId = ($arItem["CATALOG_MEASURE"]) ? $arItem["CATALOG_MEASURE"] : false;
			$arQuantityData = CMaxCustom::GetQuantityArray($totalCount, $measureId, array('ID' => $item_id), 'N', (($arItem["OFFERS"] || $arItem['CATALOG_TYPE'] == CCatalogProduct::TYPE_SET || $bSlide || !$arResult['STORES_COUNT']) ? false : true));
							...
							<div class="js_price_wrapper">
								<?if($arCurrentSKU):?>
									...
									if(isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']): // USE_PRICE_COUNT?>
										...
										<?endif;*/?>
										<?=CMaxCustom::showPriceMatrix($arCurrentSKU, $arParams, $strMeasure, $arAddToBasketData);?>
									<?else:?>
						...
						<?else:?>
							<?if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']): // USE_PRICE_COUNT?>
								...
								<?//if(count($arItem['PRICE_MATRIX']['ROWS']) > 1 || count($arItem['PRICE_MATRIX']['COLS']) > 1):?>
									<?=CMaxCustom::showPriceMatrix($arItem, $arParams, $strMeasure, $arAddToBasketData);?>
								<?//endif;?>
